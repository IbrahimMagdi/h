...
def add_fun(request):
    if request.user.is_authenticated and request.user.is_active and request.user.is_staff or request.user.is_superuser:
        ...
        if len(str(name_ar)) == 0:
            re_status = 103
            if ln == 'ar':
                re_message = 'برجاء ادخال اسم الجنسية باللغة العربية'
            else:
                re_message = 'Please enter the name of the nationality in Arabic'
            re_data = None
        elif len(str(name_ar)) < 2:
            re_status = 103
            if ln == 'ar':
                re_message = 'اسم الجنسية باللغة العربية قصير جدا'
            else:
                re_message = 'The name of the nationality in Arabic is too short'
            re_data = None
        elif len(str(name_ar)) > 48:
            re_status = 103
            if ln == 'ar':
                re_message = 'اسم الجنسية باللغة العربية كبير جدا'
            else:
                re_message = 'The name of the nationality in Arabic is too big'
            re_data = None
        # name_en
        elif len(str(name_en)) == 0:
            re_status = 103
            if ln == 'ar':
                re_message = 'برجاء ادخال اسم الجنسية الإنجليزية العربية'
            else:
                re_message = 'Please enter the name of the nationality in English'
            re_data = None
        elif len(str(name_en)) < 2:
            re_status = 103
            if ln == 'ar':
                re_message = 'اسم الجنسية باللغة الإنجليزية قصير جدا'
            else:
                re_message = 'The name of the nationality in English is too short'
            re_data = None
        elif len(str(name_en)) > 48:
            re_status = 103
            if ln == 'ar':
                re_message = 'اسم الجنسية باللغة الإنجليزية كبير جدا'
            else:
                re_message = 'The name of the nationality in English is too big'
            re_data = None
        else:
            Nationalities.objects.create(
                name_ar=name_ar,
                name_en=name_en,
                status=status,
